...
def test(f, ref_afumigatus, ref_tnf, ref_ma, ref_afumigatus_std, ref_tnf_std, ref_ma_std, times = [0, 180, 360, 720, 1440, 2145]):
    ...
    for line in f:
        ...
        if k == times[i]:
            ...
            tnf = 0 if i == 0 else math.log(float(sp[18]))
...
with open("file.tsv", "w") as outfile:
    result = subprocess.run(
        ["java", "-jar", "jPAI.jar", "15", "1000", "150", "640"],
        stdout=outfile,
        stderr=subprocess.STDOUT  # optional: combine stdout and stderr
    )
...
print("\n\n")
if pass_all_tests:
    print("jPAI.jar run and pass all tests!")
else:
    print("jPAI.jar run but failed to pass all tests!", file=sys.stderr)
