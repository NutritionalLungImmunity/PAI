...
public:
	...
	virtual void printStatistics(int k, const string& file) {
        if(k == 0) {
			ostringstream str;
            str << "Iterations\tTotal_Afumigatus\tResting_Conidia\tSwelling_Conidia\tGerminating_Conidia\tHyphae\t" <<
				"Total_TAFC\tFree-TAFC\tTAFC_Bound_to_Iron\t" <<
				"Apolactoferrin\tLactoferrin_Bound_to_Iron\tLactoferrin_Bound_to_two_Iron\t" <<
				"Total_Transferrin\tApotransferrin\tTransferrin_Bound_to_Iron\tTransferrin_Bound_to_two_Iron\t"  <<
